...
namespace MeshExplorer.Generators
{
    ...
    public class RingPolygon : BaseGenerator
    {
        ...
        public override InputGeometry Generate(double param0, double param1, double param2)
        {
            ...
            for (int i = 0; i < m; i++)
            {
                input.AddPoint(r * Math.Cos(i * step), r * Math.Sin(i * step), r * Math.Sin(i * step));
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                input.AddPoint(ro * Math.Cos(i * step + offset), ro * Math.Sin(i * step + offset), ro * Math.Sin(i * step + offset));
