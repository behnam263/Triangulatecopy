...
namespace TriangleNet.Tools
{
    ...
    public class QuadTree
    {
        ...
        public ITriangle Query(double x, double y,double z)
        {
        ...
        internal static bool IsPointInTriangle(Point p, Point t0, Point t1, Point t2)
        {
            ...
            Point d0 = new Point(t1.X - t0.X, t1.Y - t0.Y, t1.Z - t0.Z);
            Point d1 = new Point(t2.X - t0.X, t2.Y - t0.Y, t2.Z - t0.Z);
            Point d2 = new Point(p.X - t0.X, p.Y - t0.Y, p.Z - t0.Z);

            // crossproduct of (0, 0, 1) and d0
            Point c0 = new Point(-d0.Y, d0.X,d0.Z);

            // crossproduct of (0, 0, 1) and d1
            Point c1 = new Point(-d1.Y, d1.X, d1.Z);
    ...
    class QuadNode
    {
        ...
        public QuadNode(BoundingBox box, QuadTree tree, bool init)
        {
            ...
            this.bounds = new BoundingBox(box.Xmin, box.Ymin, box.Xmax, box.Ymax,box.Zmin, box.Zmax);
            this.pivot = new Point((box.Xmin + box.Xmax) / 2, (box.Ymin + box.Ymax) / 2, (box.Zmin + box.Zmax) / 2);
        ...
        public void CreateSubRegion(int currentDepth)
        {
            ...
            // 1. region south west
            box = new BoundingBox(bounds.Xmin, bounds.Ymin, pivot.X, pivot.Y, bounds.Zmin, pivot.Z);
            regions[0] = new QuadNode(box, tree);

            // 2. region south east
            box = new BoundingBox(pivot.X, bounds.Ymin, bounds.Xmax, pivot.Y, bounds.Zmin, pivot.Z);
            regions[1] = new QuadNode(box, tree);

            // 3. region north west
            box = new BoundingBox(bounds.Xmin, pivot.Y, pivot.X, bounds.Ymax, bounds.Zmin, pivot.Z);
            regions[2] = new QuadNode(box, tree);

            // 4. region north east
            box = new BoundingBox(pivot.X, pivot.Y, bounds.Xmax, bounds.Ymax, bounds.Zmin, pivot.Z);
